...
##### Calculate x, y positions per pop for testing for spatial autocorrelation
```{r}
# create UTM coordinates for each lat/long-----
# used this site: https://www.earthpoint.us/BatchConvert.aspx
# then checked manually w/this site: https://apps.waterconnect.sa.gov.au/smartforms/map.htm
utm_coords <-
  read.csv(here::here("./Urbanization_Score_files/Lat_Longs_to_UTM/80_Sites_latlongs_converted_ALL.csv"),
           header=T) %>%
  full_join(fertile, by = c("Latitude", "Longitude"))


# calculating x, y positions per population for PEDUNCLES, PODS, PODS PERPED, HEIGHT-----
groupLocations <- utm_coords %>%
  ...
  dplyr::summarise(City_dist = first(City_dist),
                   ...
                   East = first(Easting),
                   North = first(Northing),
                   pods = mean(Viable_Pods_minus1,
                               na.rm = TRUE),
                   Peduncles = mean(Peduncles_minus1,
                                    na.rm = TRUE),
                   podsperped = mean(pods_per_ped, 
                                     na.rm = TRUE),
  ...
  dplyr::summarise(City_dist = first(City_dist),
                   ...
                   pods = mean(pods, 
                               na.rm = TRUE),
                   Peduncles = mean(Peduncles,
                                    na.rm = TRUE),
                   podsperped = mean(podsperped, 
                               na.rm = TRUE),
                   height = mean(height,
                                    na.rm = TRUE))
...
# calculating x, y positions per population for POLLINIA-----
utm_coords_pollinia <-
  read.csv(here::here("./Urbanization_Score_files/Lat_Longs_to_UTM/80_Sites_latlongs_converted_ALL.csv"),
           header=T) %>%
  full_join(long_Transect_Data_18_19, by = c("Latitude", "Longitude"))

groupLocations_pollinia <- utm_coords_pollinia %>%
  dplyr::select(City_dist,
                Average_Pollinia,
                Latitude,
                Longitude,
...
DHARMa::testSpatialAutocorrelation(simulateResiduals(peds_glmer_gradient_01),
                           ...
                           y = fertile$City_dist)
# if there are multiple observations with the same x values, create first ar group with unique values for each location then aggregate the residuals per location, and calculate spatial autocorrelation on the new group


# calculating residuals per group
res2 = recalculateResiduals(simulateResiduals(
...
testSpatialAutocorrelation(res2,
                           groupLocations$East,
                           groupLocations$North)

# p = 0.01122; reject null of no spatial autocorrelation
...
### and this site: https://r-sig-mixed-models.r-project.narkive.com/wT6u4PG6/r-sig-me-spatial-autocorrelation-as-random-effect-with-count-data
### and this site: https://github.com/glmmTMB/glmmTMB/issues/552

utm_coords2 <- utm_coords %>%
  drop_na(Peduncles,
          Easting,
          Northing,
          Latitude,
...
pos <- numFactor(fertile$Latitude, fertile$Longitude)

pos_EN_centered <- numFactor(scale(utm_coords2$Easting,
                          center = TRUE,
                          scale = FALSE),
                    scale(utm_coords2$Northing,
                          center = TRUE,
                          scale = FALSE))

pos_EN <- numFactor(utm_coords2$Easting,
                    utm_coords2$Northing)


pos_EN3 <- numFactor(utm_coords2$Easting/ 1000,
                    utm_coords2$Northing/1000)


# COORDINATES IN LAT/LONG-----
peds_glmer_gradient_01.ML <- glmmTMB(Peduncles_minus1 ~ (1|Patch_ID) +
                                  City_dist:Year +
                                  City_dist +
                                  Year,
                                fertile,
...
peds_glmer_gradient_01.REML <- glmmTMB(Peduncles_minus1 ~ (1|Patch_ID) +
                                  City_dist:Year +
                                  City_dist +
                                  Year,
                                fertile,
...
### run SA model
#### model w/ exp(pos + 0|Patch_ID) doesn't converge unless Patch_ID is removed as random effect
#### model w/ mat(pos + 0|Patch_ID) doesn't converge unless Patch_ID is removed as random effect
#### model w/ gau(pos + 0|Patch_ID) isn't appropriate
peds_glmer_gradient_01_SA.exp <- glmmTMB(Peduncles_minus1 ~
                              #          (1|Patch_ID) +
                                  City_dist:Year +
                                  City_dist +
                                  Year +
...
peds_glmer_gradient_01_SA.exp.ML <- glmmTMB(Peduncles_minus1 ~
                              #          (1|Patch_ID) +
                                  City_dist:Year +
                                  City_dist +
                                  Year +
...
peds_glmer_gradient_01_SA.mat <- glmmTMB(Peduncles_minus1 ~
                                 #       (1|Patch_ID) +
                                  City_dist:Year +
                                  City_dist +
                                  Year +
...
# When (1|Patch_ID) IS removed, the ANOVA results for the models accounting for spatial autocorrelation are qualitatively identical to the original model. The AIC scores are all less than or equal to 2 AIC from the original model for each added degree of freedom.
# This is also true when the spatial autocorrelation term is changed to exp(pos + 0|City_dist) (and for the mat model).

AIC(peds_glmer_gradient_01.REML,
    peds_glmer_gradient_01_SA.exp # qualitatively identical (<=2 AIC apart per df)
    # peds_glmer_gradient_01_SA.mat
    )


# now compare ML models for ANOVA
car::Anova(peds_glmer_gradient_01_SA.exp.ML) # this is qualitatively identical to original model
car::Anova(peds_glmer_gradient_01.ML)


## try using nlme
# not sure I can do this- can't find a model that can account for zero-inflation with this package
...
# doesn't converge with or without Patch_ID as random effect
peds_glmer_gradient_01_SA.exp_EN <- glmmTMB(Peduncles_minus1 ~
                         #        (1|Patch_ID) +
                                  City_dist:Year +
                                  City_dist +
                                  Year +
        # add to account for spatial autocorrelation
       exp(pos_EN3 + 0|Patch_ID),
       
                                fertile %>%
         drop_na(Peduncles_minus1,
                 Latitude,
                 Longitude),
              zi = ~City_dist:Year + City_dist + Year,
              family = truncated_poisson,
...
# doesn't converge with or without Patch_ID as random effect
peds_glmer_gradient_01_SA.exp.ML_EN <- glmmTMB(Peduncles_minus1 ~
                               #         (1|Patch_ID) +
                                  City_dist:Year +
                                  City_dist +
                                  Year +
...
# this doesn't converge with or without Patch_ID as random effect, or REML as T or F
# peds_glmer_gradient_01_SA.mat <- glmmTMB(Peduncles_minus1 ~
#                                  # (1|Patch_ID) +
#                                   City_dist:Year +
#                                   City_dist +
...
# compare AIC w/REML models
# statistically equivalent
AIC(peds_glmer_gradient_01.REML,
    peds_glmer_gradient_01_SA.exp_EN
    )

# now compare ML models for ANOVA
# qualitatively identical
car::Anova(peds_glmer_gradient_01_SA.exp.ML_EN) 
car::Anova(peds_glmer_gradient_01.ML)
...
# running the spatial test on grouped residuals
testSpatialAutocorrelation(res2,
                           groupLocations_pollinia$East,
                           groupLocations_pollinia$North)

# p = 0.1228; can't reject null of no spatial autocorrelation
```
...
# p = 0.7314; can't reject null of no spatial autocorrelation
...
# p = 0.4744; can't reject null of no spatial autocorrelation
...
# p = 0.3432; can't reject null of no spatial autocorrelation
```
