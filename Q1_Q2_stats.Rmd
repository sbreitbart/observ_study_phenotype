...
pos_EN <- numFactor(utm_coords2$Easting, utm_coords2$Northing)
...
### run SA model
#### model w/ exp(pos + 0|Patch_ID) doesn't converge with or without Patch_ID as random effect
#### model w/ mat(pos + 0|Patch_ID) doesn't converge with or without Patch_ID as random effect
#### model w/ gau(pos + 0|Patch_ID) isn't appropriate
...
AIC(peds_glmer_gradient_01.REML,
    peds_glmer_gradient_01_SA.exp_EN # qualitatively identical (<=2 AIC apart per df)
    # peds_glmer_gradient_01_SA.mat
    )


# now compare ML models for ANOVA
car::Anova(peds_glmer_gradient_01_SA.exp.ML_EN) # this is qualitatively identical to original model
car::Anova(peds_glmer_gradient_01.ML)
