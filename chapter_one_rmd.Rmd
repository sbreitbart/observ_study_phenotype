...
city_plots <- annotate_figure(Q1_regressions_citydist, bottom = text_grob("Distance to city center (km)", size=14))
...
city_plots_subtr <- annotate_figure(Q2_regressions_citydist, bottom = text_grob("Distance to city center (km)", size=14))
...
names(div_sum)[14] <- "shannon_equit"
names(div_sum)[15] <- "simpson_equit"
...
# join urbanization scores
div_sum_no_honey <- left_join(div_sum_no_honey, AvgVars_notNA_Poll_18_19 %>%
                    dplyr::select(c(Patch_ID, Urb_score)))
...
lm_abun1 <- glm(total_indivs ~ Dist_city,
               data = div_sum,
                       family = "poisson" )
...
lm_abun1.urbsubs <- glm(total_indivs ~ Dist_city * Transect,
                       data = div_sum %>%
...
# main effects model
lm_abun1.urbsubs_ME <- glm(total_indivs ~ Dist_city + Transect,
                       data = div_sum %>%
                         filter(Transect != 'Rural'),
                       family = "poisson")
```
...
lm_abun1.u <- glm(total_indivs ~ Urb_score,
               data = div_sum,
                       family = "poisson" )
...
lm_abun1.urbsubs.u <- glm(total_indivs ~ Urb_score * Transect,
                       data = div_sum %>%
                         filter(Transect != 'Rural'),
                       family = "poisson")
...
# main effects model
lm_abun1.urbsubs.u_ME <- glm(total_indivs ~ Urb_score + Transect,
                       data = div_sum %>%
                         filter(Transect != 'Rural'),
                       family = "poisson")
```
...
## Histogram
gg_reshist(lm_diversity, bins=20)

diagnostic(lm_diversity$residuals)
# some left skew

# Try a transformation: 
gg_reshist(lm(simpson^2 ~ Dist_city, data = div_sum ), bins=15 ) # this one looks best.
gg_reshist(lm(simpson^(3) ~ Dist_city, data = div_sum ), bins=15 )
gg_reshist(lm(log(1-simpson) ~ Dist_city, data = div_sum ), bins=15) 
...
gg_reshist(lm(simpson^2 ~ Dist_city* Transect, data = div_sum %>%
                         filter(Transect != 'Rural') ), bins=15 ) # looks best
gg_reshist(lm(simpson^(3) ~ Dist_city* Transect, data =div_sum %>%
                         filter(Transect != 'Rural') ), bins=15 ) 
gg_reshist(lm(log(1-simpson) ~ Dist_city* Transect, data = div_sum %>%
                         filter(Transect != 'Rural') ), bins=15) 


lm_diversity.urbsubs <- lm(simpson^2 ~ Dist_city * Transect,
                           data = div_sum %>%
                         filter(Transect != 'Rural') )

# main effects
lm_diversity.urbsubs_ME <- lm(simpson^2 ~ Dist_city + Transect,
                           data = div_sum %>%
                         filter(Transect != 'Rural') )
...
# Try a transformation: 
gg_reshist(lm(simpson^2 ~ Urb_score, data = div_sum ), bins=15 ) 
gg_reshist(lm(simpson^(3) ~ Urb_score, data = div_sum ), bins=15 )
gg_reshist(lm(log(1-simpson) ~ Urb_score, data = div_sum ), bins=15) # this one looks best


lm_diversity.u <- lm(log(1-simpson) ~ Urb_score,
                   data = div_sum )
...
lm_diversity.urbsubs.u <- lm(simpson^2 ~ Urb_score * Transect,
                           data = div_sum %>%
                         filter(Transect != 'Rural') )

# main effects
lm_diversity.urbsubs.u_ME <- lm(simpson^2 ~ Urb_score + Transect,
                           data = div_sum %>%
                         filter(Transect != 'Rural') )
...
####### Without transects- NO TRANSFORMATION NEEDED
```{r}
lm.even <- lm(shannon_equit ~ Dist_city,
              data = div_sum )
...
lm.even.urbsubs <- lm(shannon_equit ~ Dist_city * Transect,
                      data = div_sum %>%
                        filter(Transect != "Rural") )
...
## Histogram
gg_reshist(lm.even.urbsubs, bins=20)

diagnostic(lm.even.urbsubs$residuals)
# left skew

# Try a transformation: 
gg_reshist(lm(shannon_equit ~ Dist_city* Transect,
              data = div_sum %>%
                        filter(Transect != "Rural") ),
           bins=10 ) 
gg_reshist(lm(shannon_equit^(2) ~ Dist_city* Transect,
              data = div_sum %>%
                        filter(Transect != "Rural") ),
           bins=10 )

par(mfrow=c(2,2))
plot(lm(shannon_equit^2 ~ Dist_city * Transect, data = div_sum %>%
                        filter(Transect != "Rural") ))

# Squaring makes it look much better.

lm.even.urbsubs <- lm(shannon_equit^2 ~ Dist_city * Transect,
                      data = div_sum %>%
                        filter(Transect != "Rural") )

# main effects
lm.even.urbsubs_ME <- lm(shannon_equit^2 ~ Dist_city + Transect,
                      data = div_sum %>%
                        filter(Transect != "Rural") )
```
...
####### Without transects- NO TRANSFORMATION NEEDED
```{r}
lm.even.u <- lm(shannon_equit ~ Urb_score,
              data = div_sum )
...
lm.even.urbsubs.u <- lm(shannon_equit ~ Urb_score * Transect,
                      data = div_sum %>%
                        filter(Transect != "Rural") )
...
## Histogram
gg_reshist(lm.even.urbsubs.u, bins=10)

diagnostic(lm.even.urbsubs.u$residuals)
# kurtosis a bit elevated but looks fine

# main effects
lm.even.urbsubs.u_ME <- lm(shannon_equit ~ Urb_score + Transect,
                      data = div_sum %>%
                        filter(Transect != "Rural") )
```
...
####### Transects
```{r}
car::Anova(lm_abun1.urbsubs) # not sig
...
####### All data
```{r}
car::Anova(lm_abun1.u) # highly sig

```

####### Transects
```{r}
car::Anova(lm_abun1.urbsubs.u) # interaction sig
car::Anova(lm_abun1.urbsubs.u_ME) # not sig

AIC(lm_abun1.urbsubs.u, lm_abun1.urbsubs.u_ME) # Full has sig lower AIC
```
...
######### Transects
```{r}
car::Anova(lm_diversity.urbsubs) # transect sig
car::Anova(lm_diversity.urbsubs_ME) # transect sig

AIC(lm_diversity.urbsubs, lm_diversity.urbsubs_ME) # ME has lower AIC but <2 away

summary(lm_diversity.urbsubs_ME) # corridor transect was 0.148 units diversity lower than non-corridor subtransect
```
...
######### Transects
```{r}
car::Anova(lm.diversity_nohoney.urbsubs) # transect sig
car::Anova(lm.diversity_nohoney.urbsubs_ME) # transect sig

AIC(lm.diversity_nohoney.urbsubs, lm.diversity_nohoney.urbsubs_ME) # ME has lower AIC but <2 away
...
######### Transects
```{r}
car::Anova(lm_diversity.urbsubs.u) # interaxn sig, transect marg sig, urb_score sig
car::Anova(lm_diversity.urbsubs.u_ME) #  transect marg sig, urb_score sig

AIC(lm_diversity.urbsubs.u, lm_diversity.urbsubs.u_ME) # ME has sig. lower AIC

summary(lm_diversity.urbsubs.u_ME) # corridor transect was 0.09 units diversity lower than non-corridor subtransect
```
...
######### Transects
```{r}
car::Anova(lm.diversity_nohoney.urbsubs.u) # interaxn, transect, urb_score sig
car::Anova(lm.diversity_nohoney.urbsubs.u_ME) # transect, urb_score sig

AIC(lm.diversity_nohoney.urbsubs.u, lm.diversity_nohoney.urbsubs.u_ME) # full model has sig. lower AIC
```
...
# urban sites had, on average, 0.47 units higher richness than rural!
```


####### Transects
```{r}
car::Anova(lm.rich.urbsubs) # transect sig
car::Anova(lm.rich.urbsubs_ME) # transect sig

AIC(lm.rich.urbsubs, lm.rich.urbsubs_ME) # ME lower AIC but <2

summary(lm.rich.urbsubs_ME)

# corridor subtransect had 0.34 units lower richness than non-corridor, on avg!
```
...
####### Transects
```{r}
car::Anova(lm.rich.urbsubs.u) # interaxn marg sig, transect & urb_score sig
car::Anova(lm.rich.urbsubs.u_ME) # transect & urb_score sig

AIC(lm.rich.urbsubs.u, lm.rich.urbsubs.u_ME) # full model lower AIC but <2
```
...
####### Transects
```{r}
car::Anova(lm.even.urbsubs) # transect sig
car::Anova(lm.even.urbsubs_ME) # transect sig

AIC(lm.even.urbsubs,lm.even.urbsubs_ME ) # full model lower AIC but <2

summary(lm.even.urbsubs) # corridor subtransect had 0.636 units lower evenness than non-corridor, on avg!
...
####### Transects
```{r}
car::Anova(lm.even.urbsubs.u) # interaxn and urb_score sig
car::Anova(lm.even.urbsubs.u_ME) # urb_score sig

AIC(lm.even.urbsubs.u,lm.even.urbsubs.u_ME ) # full model sig lower AIC
...
#doing this one manually (not w/a function) b/c it has glm and lm's
lapply(best_mods_Q3.dist_list, tidy_sb.pollinators_lm) %>%
    ...
    add_header_row(
      ...
      colwidths = c(1, 3, 4, 4)) %>%
    align(j = 1, align = "left", part = "all") %>%
    align(j = 2:12, align = "center", part = "body") %>%
    align(j = 2:12, align = "center", part = "header") %>%
    display(i = 2, col_key = "X2", 
      pattern = "{{val}}{{pow}}", part = "header",
      formatters = list(val ~ as.character("X"), pow ~ as.character("2") ),
      fprops = list(pow = fp_text(vertical.align = "superscript", font.size = 12))) %>%
      ...
      fontsize(size = 12, part = "body") %>%
  width(j = 1, width = 2.3) %>%
  width(j = c(2,4), width = 0.7) %>%
  width(j = c(3, 6, 10), width = 0.2) %>%
  width(j = c(5, 9), width = 1.2) %>%
  width(j = c(7:8, 11:12), width = 0.7) %>%
  italic(i = 2, j = 2:12, part = "header") %>%
  add_footer_lines("Table X: Results from general and generalized linear models examining the effects of urbanization on pollinator abundance, diversity, and richness. Urbanization was quantified via distance to city center and all populations were included. Shown are maximum likelihood chi-square (X2), degrees of freedom (df), p-values (p), sums of squares, and F-statistics obtained from type II ANOVA.") %>%
...
lapply(best_mods_Q3urban.dist_list, tidy_sb.pollinators_lm) %>%
  ...
  width(j = c(7:8, 11:12), width = 0.7) %>%
  italic(i = 2, j = 2:12, part = "header") %>%
  add_footer_lines("Table X: Results from general and generalized linear models examining the effects of urbanization and distance to an urban corridor on pollinator abundance, diversity, and richness. Urbanization was quantified via distance to city center and only urban populations were included. Shown are maximum likelihood chi-square (X2), degrees of freedom (df), p-values (p), sums of squares, and F-statistics obtained from type II ANOVA.") %>%
  add_footer_lines(paste0("AIC, Abundance: ",
                          AIC(lm_abun1.urbsubs_ME) %>% round(., 3))) %>%
  add_footer_lines(paste0("AIC, Diversity: ",
                          AIC(lm_diversity.urbsubs_ME) %>% round(., 3))) %>%
    add_footer_lines(paste0("AIC, Richness: ",
                          AIC(lm.rich.urbsubs_ME) %>% round(., 3))) %>%
  add_footer_lines("*: p-value <= 0.05") %>%
  add_footer_lines("**: p-value <= 0.01") %>%
  add_footer_lines("***: p-value <= 0.001") %>%
  save_as_image(., path = here::here("./Figures_Tables/Q3_ANOVA/Q3_Dist_urban_ANOVA.png"))
...
lapply(best_mods_Q3.usc_list, tidy_sb.pollinators_lm) %>%
  ...
  width(j = c(7:8, 11:12), width = 0.7) %>%
  italic(i = 2, j = 2:12, part = "header") %>%
  add_footer_lines("Table X: Results from general and generalized linear models examining the effects of urbanization on pollinator abundance, diversity, and richness. Urbanization was quantified via urbanization score and all populations were included. Shown are maximum likelihood chi-square (X2), degrees of freedom (df), p-values (p), sums of squares, and F-statistics obtained from type II ANOVA.") %>%
  add_footer_lines(paste0("AIC, Abundance: ",
...
lapply(best_mods_Q3urban.usc_list, tidy_sb.pollinators_lm) %>%
  ...
  width(j = c(7:8, 11:12), width = 0.7) %>%
  italic(i = 2, j = 2:12, part = "header") %>%
  add_footer_lines("Table X: Results from general and generalized linear models examining the effects of urbanization and distance to an urban corridor on pollinator abundance, diversity, and richness. Urbanization was quantified via urbanization score and only urban populations were included. Shown are maximum likelihood chi-square (X2), degrees of freedom (df), p-values (p), sums of squares, and F-statistics obtained from type II ANOVA.") %>%
  add_footer_lines(paste0("AIC, Abundance: ",
                          AIC(lm_abun1.urbsubs.u) %>% round(., 3))) %>%
  add_footer_lines(paste0("AIC, Diversity: ",
                          AIC(lm_diversity.urbsubs.u_ME) %>% round(., 3))) %>%
    add_footer_lines(paste0("AIC, Richness: ",
                          AIC(lm.rich.urbsubs.u) %>% round(., 3))) %>%
  add_footer_lines("*: p-value <= 0.05") %>%
  add_footer_lines("**: p-value <= 0.01") %>%
  add_footer_lines("***: p-value <= 0.001") %>%
  save_as_image(., path = here::here("./Figures_Tables/Q3_ANOVA/Q3_Usc_urban_ANOVA.png"))
...
tidy_sb.pollinators_lm(lm.even) %>%
  ...
  italic(i = 2, j = 2:5, part = "header") %>%
  add_footer_lines("Table X: Results from a general linear model examining the effects of urbanization on pollinator evenness. Urbanization was quantified via distance to city center and all populations were included. Shown are sums of squares, degrees of freedom (df), F-statistics, and p-values (p) obtained from type II ANOVA.") %>%
...
tidy_sb.pollinators_lm(lm.even.urbsubs) %>%
  ...
  width(j = c(3,4,5), width = 0.2) %>%
  italic(i = 2, j = 2:5, part = "header") %>%
  add_footer_lines("Table X: Results from a general linear model examining the effects of urbanization and distance to an urban corridor on pollinator evenness. Urbanization was quantified via distance to city center and only urban populations were included. Shown are sums of squares, degrees of freedom (df), F-statistics, and p-values (p) obtained from type II ANOVA.") %>%
  add_footer_lines(paste0("AIC, Evenness: ",
                          AIC(lm.even.urbsubs) %>% round(., 3))) %>%
  add_footer_lines("*: p-value <= 0.05") %>%
  add_footer_lines("**: p-value <= 0.01") %>%
  add_footer_lines("***: p-value <= 0.001") %>%
  save_as_image(., path = here::here("./Figures_Tables/Q3_ANOVA/Supplement/Best_models_Evenness/Q3_Dist_urb_evenness_ANOVA.png"))
...
tidy_sb.pollinators_lm(lm.even.u) %>%
  ...
  width(j = c(3,4,5), width = 0.2) %>%
  italic(i = 2, j = 2:5, part = "header") %>%
  add_footer_lines("Table X: Results from a general linear model examining the effects of urbanization on pollinator evenness. Urbanization was quantified via urbanization score and all populations were included. Shown are sums of squares, degrees of freedom (df), F-statistics, and p-values (p) obtained from type II ANOVA.") %>%
  add_footer_lines(paste0("AIC, Evenness: ",
...
tidy_sb.pollinators_lm(lm.even.urbsubs.u) %>%
    as.data.frame() %>%
    # purrr::reduce(full_join, by = "Predictor") %>%
    arrange(factor(Predictor, levels = c("Distance to City Center",
                                         "Urbanization Score",
  ...
  add_footer_lines("**: p-value <= 0.01") %>%
  add_footer_lines("***: p-value <= 0.001") %>%
  save_as_image(., path = here::here("./Figures_Tables/Q3_ANOVA/Supplement/Best_models_Evenness/Q3_Urbscore_urb_evenness_ANOVA.png"))
```
...
lapply(alt_best_mods_Q3.dist_list, tidy_sb.pollinators_lm) %>%
    ...
    display(i = 2, col_key = "X2", 
      ...
      fontsize(size = 12, part = "body") %>%
  width(j = 1, width = 3.4) %>%
  width(j = 2:12, width = 0.7) %>%
  italic(i = 2, j = 2:12, part = "header") %>%
  add_footer_lines("Table X: Results from alternative general linear mixed models within 2 AIC from the best model examining the effects of urbanization and proximity to an urban corridor on pollinator abundance, diversity, and richness. Urbanization was quantified via distance to city center and only urban populations were included. Shown are maximum likelihood chi-square (X2), degrees of freedom (df), p-values (p), sums of squares, and F-statistics obtained from type II ANOVA.") %>%
  add_footer_lines(paste0("AIC, Inflorescences, Alt. Model 1: ",
                          AIC(lm_abun1.urbsubs) %>% round(., 3))) %>%  add_footer_lines(paste0("AIC, Inflorescences, Alt. Model 1: ",
                          AIC(lm_diversity.urbsubs) %>% round(., 3))) %>%  add_footer_lines(paste0("AIC, Pollinaria Removed, Alt. Model 1: ",
...
## Urb_score-----
# abundance & diversity: no alt best model
...
lm.rich.urbsubs.u_ME %>%
  ...
  italic(i = 2, j = 2:5, part = "header") %>%
  add_footer_lines("Table X: Results from an alternative general linear mixed model within 2 AIC from the best model examining the effects of urbanization and proximity to an urban corridor on pollinator richness. Alternative models of other pollinator indices were >2 AIC of the best models and are not shown. Urbanization was quantified via urbanization score and only urban populations were included. Shown are sums of squares, degrees of freedom (df), F-statistics, and p-values (p) obtained from type II ANOVA.") %>%
  add_footer_lines(paste0("AIC, Richness, Alt. Model 1: ",
...
lm.even.urbsubs_ME %>%
  ...
  add_footer_lines("**: p-value <= 0.01") %>%
  add_footer_lines("***: p-value <= 0.001") %>%
  save_as_image(., path = here::here("./Figures_Tables/Q3_ANOVA/Supplement/Alternative_Models/Alt_models_evenness/Q3_Dist_ANOVA_altmodels_evenness.png"))
...
Morphospecies_table <- colSums(pollinators[,c(7:48)]) %>%
  ...
  # dplyr::rename(Morphospecies_Code = 0) %>%
  dplyr::rename(Individuals_observed = 2) %>%
  # as.data.frame() %>%
  left_join(., diversity %>%
              dplyr::select(c(1,14)),
            by = "Morphospecies_Code") %>%
  write.csv(here::here("./Figures_Tables/Q3_Pollinator/Q3_Pollinators_Table.csv"))
...
four_popfams <- four_popfams[,c(2,1,3,4)] %T>%
  write.csv(here::here("./Figures_Tables/Q3_Pollinator/Q3_4MostCommonMorphospecies_Table.csv")) 
...
four_popfams_summary <- four_popfams %>%
  ...
  write.csv(here::here("./Figures_Tables/Q3_Pollinator/Q3_4MostCommonMorphospecies_SummaryTable.csv")) 
...
```{r}
# Total abundance
abun <- ggplot(pollinators,
               aes(x = Dist_city,
                   y = Total)) +
  labs(x = "Distance to city center (km)",
       y = "Average Pollinators Observed") +
  geom_point() +
  scale_shape(solid = F)+
...
abun_transect <- ggplot(pollinators,
                        aes(x = Dist_city,
                            y = Total,
                            shape = Transect,
                            color = Transect)) +
  labs(x = "Distance to city center (km)", y = "Average Pollinators Observed") +
  ...
  geom_smooth(aes(color=Transect,
                  linetype = Transect,
                  fill = Transect),
              method = lm, alpha=0.15,  se = T) +
  labs(title = 'Pollinator Abundance Among Subtransects',
       linetype="Transect",
       color="Transect",
       linetype = "Transect") +
  theme(
...
abun_perplant <- ggplot(poll_pp, aes(x = Dist_city, y = Total)) +
  labs(x = "Distance to city center (km)", y = "Average Pollinators Observed (per plant)") +
  geom_point() +
  scale_shape(solid = F)+
  geom_smooth(color="black", method = lm, alpha=0.15,  se = T) +
...
ggplot(subset(diversity, Family %in% c('Apidae', 'Cantharidae', 'Halictidae', 'Megachilidae')), aes(x = Dist_city, y = Count, colour = Family)) +
  labs(x = "Distance to city center (km)", y = "Individuals Observed by Family (per population)") +
...
###### Honeybee abundance re: dist city ctr #######
  ...
  labs(x = "Distance to city center (km)", y = "Honeybees Observed (per population)") +
  geom_point() +
  scale_shape(solid = F)+
  geom_smooth( method = lm, alpha=0.15,  se = T) +
  labs()
...
ggplot(poll_binary, aes(x = Dist_city, y = Total)) +
  labs(x = "Distance to city center (km)", y = "Morphospecies Observed") +
  geom_point() +
  scale_shape(solid = F)+
  geom_smooth(color="black", method = lm, alpha=0.15,  se = T) +
...
ggplot(poll_binary, aes(x = Dist_city, y = Total, shape=Transect, color = Transect)) +
  labs(x = "Distance to city center (km)", y = "Morphospecies Observed (per population)") +
  geom_point(aes(shape=Transect, color=Transect)) +
  scale_shape(solid = F)+
...
ord_trsct <- ggplot(div_sum, aes(x = Dist_city, y = orders, shape = Transect, colour = Transect)) +
  labs(x = "Distance to city center (km)", y = "Orders") +
  geom_point(aes(shape = Transect, color = Transect)) +
  scale_shape(solid = F)+
...
supfam_trsct <- ggplot(div_sum, aes(x = Dist_city, y = superfams, shape = Transect, colour = Transect)) +
  labs(x = "Distance to city center (km)", y = "Superfamilies") +
  geom_point(aes(shape = Transect, color = Transect)) +
  scale_shape(solid = F)+
  geom_smooth(aes(shape = Transect, color = Transect), method = lm, alpha=0.15,  se = T) +
  labs() +
...
mspecies_trsct <- ggplot(div_sum, aes(x = Dist_city, y = mspecies, shape = Transect, colour = Transect)) +
  labs(x = "Distance to city center (km)", y = "Morphospecies") +
  geom_point(aes(shape = Transect, color = Transect)) +
  scale_shape(solid = F)+
  geom_smooth(aes(shape = Transect, color = Transect), method = lm, alpha=0.15,  se = T) +
  labs() +
...
ggplot(subset(div_sum_melted, variable %in% c( 'shannon', 'simpson')), aes(x = Dist_city, y = value, shape = variable, color = variable)) +
  ...
  scale_linetype_discrete(name  ="Diversity Index",
                            labels=c("Shannon", "Simpson")) +
  labs(x = "Distance to city center (km)", y = "Diversity") +
  geom_point() +
...
ggplot(div_sum_melted, aes(x = Dist_city, y = value)) +
  labs(x = "Distance to city center (km)", y = "Diversity Index") +
...
rich_fig1 <- ggplot(subset(div_sum_melted, variable %in% c('Menhinicks_index')), aes(x = Dist_city, y = value), shape = 1, color = "black") +
  labs(x = "Distance to city center (km)", y = "Species Richness") +
...
even_fig1 <- ggplot(subset(div_sum_melted, variable %in% c( 'shannon_equit', 'simpson_equit')), aes(x = Dist_city, y = value, shape = variable, color = variable)) +
  labs(x = "Distance to city center (km)", y = "Equitability") +
  geom_point() +
  scale_shape(solid = F)+
  geom_smooth( method = lm, alpha=0.15,  se = T) +
...
annotate_figure(Q2_regressions, bottom = text_grob("Distance to city center (km)", size=14))
