...
ms_table <- colSums(pollinators[,c(7:48)]) %>%
  as.data.frame() %>%
  dplyr::rename(indivs_observed = 1) %T>%
  view()
...
four_popfams <- dplyr::filter(diversity,
  ...
  dplyr::summarise(
    ...
    "Individuals_observed" = sum(Count)) %T>%
  view()
...
four_popfams_summary <- four_popfams %>%
  ...
  arrange(desc(Individuals_observed))
#   
...
library(Rmisc)
sum1 <- summarySE(diversity, measurevar="Count",
                  groupvars= c("Order", "Transect"),
                  na.rm = T)
