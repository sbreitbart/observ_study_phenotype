...
## Histogram
gg_reshist(lm.even.urbsubs, bins=15)
...
######## Transects
```{r}
car::Anova(lm_abun1.urbsubs.u) # not sig
car::Anova(lm_abun1.urbsubs.u_ME) # not sig

AIC(lm_abun1.urbsubs.u, lm_abun1.urbsubs.u_ME) # Full has lower AIC (not <2 away though)
```
...
######## Transects
```{r}
car::Anova(lm_abun1.urbsubs.nohoney) # not sig
...
######## Transects
```{r}
car::Anova(lm_abun1.urbsubs.u_nohoney) # interaxn marg sig
car::Anova(lm_abun1.urbsubs.u_nohoney_ME) # not sig
...
######### Transects
```{r}
car::Anova(lm_diversity.urbsubs) # transect marg sig
car::Anova(lm_diversity.urbsubs_ME) # transect marg sig

AIC(lm_diversity.urbsubs, lm_diversity.urbsubs_ME) # ME has lower AIC but <2 away

summary(lm_diversity.urbsubs_ME) # corridor transect was 0.109 units diversity lower than non-corridor subtransect
```
...
AIC(lm.diversity_nohoney.urbsubs, lm.diversity_nohoney.urbsubs_ME) # full model has lower AIC but <2 away

summary(lm.diversity_nohoney.urbsubs) # corridor transect was 0.270 units diversity lower than non-corridor subtransect
```
...
summary(lm_diversity.urbsubs.u_ME) # corridor transect was 0.093 units diversity lower than non-corridor subtransect
```
...
# corridor subtransect had 1.199 units lower richness than non-corridor, on avg!
```
...
####### Transects
```{r}
car::Anova(lm.even.urbsubs) # transect sig, interaxn marg sig
car::Anova(lm.even.urbsubs_ME) # transect sig

AIC(lm.even.urbsubs,lm.even.urbsubs_ME ) # full model lower AIC but <2

summary(lm.even.urbsubs) # corridor subtransect had 0.615 units lower evenness than non-corridor, on avg!
...
lapply(best_mods_Q3urban.dist_list_NOHONEY, tidy_sb.pollinators_lm) %>%
  ...
  add_footer_lines("**: p-value <= 0.01") %>%
  add_footer_lines("***: p-value <= 0.001") %>%
  save_as_image(., path = here::here("./Figures_Tables/Q3_ANOVA/Supplement/Models_without_Honeybees/Q3_Dist_urban_ANOVA_nohoneybees.png"))
...
lapply(best_mods_Q3urban.usc_list_NOHONEY, tidy_sb.pollinators_lm) %>%
  ...
  add_footer_lines("**: p-value <= 0.01") %>%
  add_footer_lines("***: p-value <= 0.001") %>%
  save_as_image(., path = here::here("./Figures_Tables/Q3_ANOVA/Supplement/Models_without_Honeybees/Q3_Urbscore_urban_ANOVA_nohoneybees.png"))
...
lapply(alt_best_mods_Q3.dist_list, tidy_sb.pollinators_lm) %>%
  ...
  add_footer_lines(paste0("AIC, Abundance, Alt. Model 1: ",
                          ...
                          AIC(lm_diversity.urbsubs) %>% round(., 3))) %>%  add_footer_lines(paste0("AIC, Richness, Alt. Model 1: ",
