...
### Create plots
#### Include points and stats
```{r}
# FIRST, find population-level means
# separate df with this data for pollinia
fertile_pop_means <- fertile %>%
  dplyr::group_by(Year, Patch_ID) %>%
  dplyr::summarise(peds = mean(Peduncles,
                               na.rm = T),
                   pods = mean(Viable_Pods,
                               na.rm = T),
                   pods_per_ped = mean(pods_per_ped,
                               na.rm = T),
                   height = mean(Height_Sept,
                                 na.rm = T),
...
ggpred_Q1.citydist_peds <- (ggplot(peds_gradient_01_pred) +
    ...
    geom_ribbon(aes(x = x,
                    ...
                    fill = group),
                alpha = 0.3) +
    geom_point(data = fertile_pop_means,
               aes(x = City_dist,
                   y = peds,
                   colour = Year,
                   shape = Year),
...
ggpred_Q1.citydist_pods <- (ggplot(pods_gradient_01_pred) +
  ...
  # adding stats from ANOVA
           # geom_text_npc(aes(npcx = 0.05, npcy = 1,
...
# dev.copy2pdf(file="~/R_Projects/chapter_one/Figures_Tables/Q1_Regressions_Plants/DISTGradient_regressions_nopoints.pdf",
#              width = 12.5, height = 3.75)

dev.copy2pdf(file="~/R_Projects/chapter_one/Figures_Tables/Q1_Regressions_Plants/DISTGradient_regressions_WITHpoints.pdf",
             width = 12.5, height = 3.75)
```
...
dev.copy2pdf(file="~/R_Projects/chapter_one/Figures_Tables/Q1_Regressions_Plants/URBSCOREGradient_regressions_points.pdf",
             width = 12, height = 7)
...
##### Original version
```{r}
levels(peds_subtr_01_pred$facet)
...
ggpred_Q2.citydist_peds <-
  ...
  # theme(legend.position = "none",#c(.99, .99),
  #       legend.justification = c("right", "top"),
  #       legend.box = 'horizontal',
  #       legend.box.just = "right",
  #       legend.margin = margin(5,5,5,5),
...
ggpred_Q2.citydist_peds <-
  ...
  labs(x = "Distance to Urban Center (km)",
       y = "Inflorescences") +
  theme(legend.position =c(.99, .99), #"none",
        legend.justification = c("right", "top"),
        legend.box = 'horizontal',
        legend.box.just = "right",
        legend.margin = margin(5,5,5,5),
...
ggpred_Q2.citydist_poll <- (ggplot(poll_subtr_01_pred) +
  ...
  scale_fill_brewer(palette = "RdBu") +
  scale_shape(solid = FALSE) +
  scale_y_continuous(breaks=c(0,1,2,3), limits=c(NA,3)) +
  labs(x = "Distance to Urban Center (km)",
       y = "Pollinaria Removed") +
  theme(legend.position = c(.99, .99),#"none",
        legend.justification = c("right", "top"),
...
levels(pods_subtr_01_pred$facet)
ggpred_Q2.citydist_pods <- (ggplot(pods_subtr_01_pred) +
                              geom_smooth(aes(x = x,
                 y = predicted,
                 color = interaction(group, facet),
...
## factors (transects) for this object are North, then South
## so to keep legend consistent, have to reorder levels first
podsperped_subtr_01_pred$facet <- factor(podsperped_subtr_01_pred$facet,  levels(podsperped_subtr_01_pred$facet)[c(2,1)])
...
ggpred_Q2.citydist_height <- (ggplot(height_subtr_01_pred) +                                geom_smooth(aes(x = x,
                 y = predicted,
                 color = interaction(group, facet),
                 fill = interaction(group, facet),
                 linetype = (group)
...
### Create table of ggpredict objects for supplement
```{r}
# Create one table with 4 variables-----
## can't add suffixes if I try to join 2 tables, then a third, then a fourth, so I'll create 2 tables with suffixes for variables, then join those
## DO SEPARATELY FOR POLLINIA- MORE ESTIMATES THAN OTHER VARIABLES


# For pods, pods per ped, and peds
urbsc_estimates1 <- full_join(
                            # poll_subtr_01_pred.u,
                             pods_subtr_01_pred.u,
                             podsperped_subtr_01_pred.u,
                             by = c("x", "group", "facet"),
                             suffix = c(#"_poll",
                                        "_pods",
                                        "_ppp"))

urbsc_estimates <- full_join(urbsc_estimates1,
                             peds_subtr_01_pred.u,
                             by = c("x", "group", "facet")) %>%
  dplyr::relocate(group, .after = 1) %>%
  dplyr::relocate(facet, .after = 2) %>%
  dplyr::rename("Urbanization Score" = 1,
                "Year" = 2,
                "Subtransect" = 3) %>%
  dplyr::mutate(Subtransect = case_when(Subtransect == "South" ~ "Urban: Corridor",
                                        Subtransect == "North" ~ "Urban: Non-Corridor")) %>%
  dplyr::select(-starts_with("conf")) %>% # remove confidence interval cols
  dplyr::mutate_at(4:9, round, 3) %>%
  dplyr::rename("Estimate" = 4,
                "Estimate " = 6,
                "Estimate  " = 8,
                "SE" = 5,
                "SE " = 7,
...
save_as_html("Urbanization Score Estimates" = urbsc_estimates,
             path = here::here("./Figures_Tables/Q2_Estimates_Plants/3_estimates.html"))



# For pollinia
urbsc_estimates_pollinia <- poll_subtr_01_pred.u %>%
  dplyr::relocate(group, .after = 1) %>%
  dplyr::relocate(facet, .after = 2) %>%
  dplyr::rename("Urbanization Score" = 1,
                "Year" = 2,
  ...
  add_header_row(
    ...
    align(align = "center", part = "all") %>%
  autofit()
urbsc_estimates_pollinia

save_as_html("Urbanization Score Estimates" = urbsc_estimates_pollinia,
             path = here::here("./Figures_Tables/Q2_Estimates_Plants/Pollinia_estimates.html"))
...
```{r}
# peduncles-----
levels(peds_subtr_01_pred.u$facet)
ggpred_Q2.urbscore_peds <- (ggplot(peds_subtr_01_pred.u) +
         geom_smooth(aes(x = x,
                 y = predicted,
                 color = interaction(group, facet),
...
# pods-----
levels(pods_subtr_01_pred.u$facet)
...
## for some reason (peduncles and at least one more is South, North)
## so to keep legend consistent, have to reorder levels first
podsperped_subtr_01_pred.u$facet <- factor(podsperped_subtr_01_pred.u$facet,                                     levels(podsperped_subtr_01_pred.u$facet)[c(2,1)])

levels(podsperped_subtr_01_pred.u$facet)

ggpred_Q2.urbscore_podsperped <- (ggplot(podsperped_subtr_01_pred.u) +    geom_smooth(aes(x = x,
                 y = predicted,
                 color = interaction(group, facet),
                 fill = interaction(group, facet),
                 linetype = (group)
...
ggpred_Q2.citydist_height.u <- (ggplot(height_subtr_01_pred.u) +
  ...
  labs(x = "Urbanization Score",
       y = "Height (cm)") +
  theme(legend.position =  c(.97, .99),# "none",
    legend.justification = c("center"),
    legend.box = 'horizontal',
    legend.box.just = "center",
    legend.margin = margin(5,5,5,5),
...
library(patchwork)
library(png)
Q2_legend <- readPNG(here::here("./Figures_Tables//Q2_Regressions_Plants/Q2_legend_points_revised.png"),
                    native = TRUE)
...
# export to PDF
dev.copy2pdf(file="~/R_Projects/chapter_one/Figures_Tables//Q2_Regressions_Plants/DISTSubtransect_regressions_points.pdf",
             width = 12, height = 4.5)
```
...
# library(patchwork)
# Q2_legend <- readPNG(here::here("./Figures_Tables//Q2_Regressions_Plants/Q2_legend.png"),
#                      native = TRUE)
# 
# 
...
# 
# # export to PDF
# dev.copy2pdf(file="~/R_Projects/chapter_one/Figures_Tables//Q2_Regressions_Plants/URBSCORESubtransect_regressions_ggpredict.pdf",
#              width = 12, height = 4.5)
...
library(patchwork)
library(png)
Q2_legend <- png::readPNG(here::here("./Figures_Tables/Q2_Regressions_Plants/Q2_legend_points_revised.png"),
                     native = TRUE)
...
# export to PDF
dev.copy2pdf(file="~/R_Projects/chapter_one/Figures_Tables//Q2_Regressions_Plants/URBSCORESubtransect_regressions_ggpredict_points.pdf",
             width = 12, height = 8.5)
```
