...
##### City_dist
```{r}
peds_glm_gradient_01 <- glmmTMB(Peduncles ~ City_dist * Year, 
                data = fertile_pops_all,
                family = "poisson")

plot(simulateResiduals(
peds_glm_gradient_01
)) # also overdispersed... try neg binomial


# plot(simulateResiduals(
# update(peds_glm_gradient_01, .~., family = nbinom1())
# )) # better... try squaring now
# 
# 
# plot(simulateResiduals(
# update(peds_glm_gradient_01, Peduncles^2 ~., family = nbinom1())
# )) # didn't help. going with model above


# Full model
peds_glm_gradient_01 <- update(peds_glm_gradient_01, .~., family = nbinom1())

# Reduced model
peds_glm_gradient_02 <- update(peds_glm_gradient_01, .~ City_dist + Year)
...
u_peds_glm_gradient_01 <- glmmTMB(Peduncles ~ Urb_score * Year, 
                data = fertile_pops_all,
                family = "poisson")

# 
# plot(simulateResiduals(
# u_peds_glm_gradient_01
# )) # also overdispersed... try neg binomial
# 
# 
# plot(simulateResiduals(
# update(u_peds_glm_gradient_01, .~., family = nbinom1())
# )) # better... try squaring now


# plot(simulateResiduals(
# update(u_peds_glm_gradient_01, Peduncles^2 ~., family = nbinom1())
# )) # didn't help. going with model above


# Full model
u_peds_glm_gradient_01 <- update(u_peds_glm_gradient_01, .~., family = nbinom1())

# Reduced model
u_peds_glm_gradient_02 <- update(u_peds_glm_gradient_01, .~ Urb_score + Year)
...
##### City_dist
```{r}
peds_glm_gradient_list <- list(peds_glm_gradient_01,
                                 peds_glm_gradient_02)
...
for (i in 1:length(peds_glm_gradient_list)) {
  formulas_peds[i, 1] <- toString(i)
  formulas_peds[i, 2] <- toString(formula(peds_glm_gradient_list[[i]]))
  # formulas_peds[i, 3] <- r.squaredGLMM(peds_glm_gradient_list[[i]])[1,1] %>%
  #   round(digits = 3)
  # formulas_peds[i, 4] <- r.squaredGLMM(peds_glm_gradient_list[[i]])[1,2] %>%
  #   round(digits = 3)
}

glance_peds <- do.call(rbind,
                       lapply(peds_glm_gradient_list,
                              broom::glance)) %>%
  as.data.frame() %>%
...
bound_peds <- bind_cols(formulas_peds,
                        glance_peds %>%
                          dplyr::select(AIC, BestModels)) # Model 1 best AIC


summary(peds_glm_gradient_01)
peds_glm_gradient_01_anova <- car::Anova(peds_glm_gradient_01) %T>%
  print() # interaxn sig
...
##### urb_score
```{r}
u_peds_glm_gradient_list <- list(u_peds_glm_gradient_01,
                                 u_peds_glm_gradient_02)

u_formulas_peds <- data.frame(matrix(ncol = 0, nrow = 2))
u_formulas_peds$Model <- NA
u_formulas_peds$Formula <- NA
# u_formulas_peds$r.squaredGLMM_R2m <- NA
# u_formulas_peds$r.squaredGLMM_R2c <- NA



for (i in 1:length(u_peds_glm_gradient_list)) {
  u_formulas_peds[i, 1] <- toString(i)
  u_formulas_peds[i, 2] <- toString(formula(u_peds_glm_gradient_list[[i]]))
  # u_formulas_peds[i, 3] <- r.squaredGLMM(u_peds_glm_gradient_list[[i]])[1,1] %>%
  #   round(digits = 3)
  # u_formulas_peds[i, 4] <- r.squaredGLMM(u_peds_glm_gradient_list[[i]])[1,2] %>%
  #   round(digits = 3)
}

u_glance_peds <- do.call(rbind,
                       lapply(u_peds_glm_gradient_list,
                              broom::glance)) %>%
  as.data.frame() %>%
  mutate(BestModels = case_when(
    AIC == min(AIC, na.rm=T) ~ "*",
...
u_bound_peds <- bind_cols(u_formulas_peds,
                        u_glance_peds %>%
                          dplyr::select(AIC, BestModels)) # Models 1, 2 best AIC (1 is lowest through)


summary(u_peds_glm_gradient_01)
u_peds_glm_gradient_01_anova <- car::Anova(u_peds_glm_gradient_01) %T>%
...
##### City_dist
```{r}
peds_glm_subtr_01 <- glmmTMB(Peduncles ~ City_dist * Year * Transect_ID, 
                data = fertile_pops_all %>%
                  filter(Transect_ID != "Rural"),
                family = "poisson")
...
# plot(simulateResiduals(
# peds_glm_subtr_01
# )) # looks good! try neg bin just to see
# 
# 
# plot(simulateResiduals(
# update(peds_glm_subtr_01, .~., family = nbinom1())
# )) # not much different
# 
# 
# plot(simulateResiduals(
# update(peds_glm_subtr_01, Peduncles^2 ~., family = nbinom1())
# )) # not much different. going with original model


# all main effects and secondary interactions
peds_glm_subtr_02 <- update(peds_glm_subtr_01,
                              . ~ . -City_dist:Year:Transect_ID)

# all main effects and 2/3 secondary interactions
peds_glm_subtr_03 <- update(peds_glm_subtr_02,
                              . ~ . -City_dist:Transect_ID)
peds_glm_subtr_04 <- update(peds_glm_subtr_02,
                              . ~ . -City_dist:Year)
peds_glm_subtr_05 <- update(peds_glm_subtr_02,
                              . ~ . -Year:Transect_ID)

# all main effects and 1/3 secondary interactions
peds_glm_subtr_06 <- update(peds_glm_subtr_03,
                              . ~ . -City_dist:Year)
peds_glm_subtr_07 <- update(peds_glm_subtr_04,
                              . ~ . -Year:Transect_ID)
peds_glm_subtr_08 <- update(peds_glm_subtr_05,
                              . ~ . -City_dist:Transect_ID)

# all main effects
peds_glm_subtr_09 <- update(peds_glm_subtr_08,
                              . ~ . -City_dist:Year)
...
u_peds_glm_subtr_01 <- glmmTMB(Peduncles ~ Urb_score * Year * Transect_ID, 
                data = fertile_pops_all %>%
                  filter(Transect_ID != "Rural"),
                family = "poisson")
...
# plot(simulateResiduals(
# u_peds_glm_subtr_01
# )) # overdispersed... try neg bin
# 
# 
# plot(simulateResiduals(
# update(u_peds_glm_subtr_01, .~., family = nbinom1())
# )) # better! will go with this one


u_peds_glm_subtr_01 <- update(u_peds_glm_subtr_01, .~., family = nbinom1())


# all main effects and secondary interactions
u_peds_glm_subtr_02 <- update(u_peds_glm_subtr_01,
                              . ~ . -Urb_score:Year:Transect_ID)

# all main effects and 2/3 secondary interactions
u_peds_glm_subtr_03 <- update(u_peds_glm_subtr_02,
                              . ~ . -Urb_score:Transect_ID)
u_peds_glm_subtr_04 <- update(u_peds_glm_subtr_02,
                              . ~ . -Urb_score:Year)
u_peds_glm_subtr_05 <- update(u_peds_glm_subtr_02,
                              . ~ . -Year:Transect_ID)

# all main effects and 1/3 secondary interactions
u_peds_glm_subtr_06 <- update(u_peds_glm_subtr_03,
                              . ~ . -Urb_score:Year)
u_peds_glm_subtr_07 <- update(u_peds_glm_subtr_04,
                              . ~ . -Year:Transect_ID)
u_peds_glm_subtr_08 <- update(u_peds_glm_subtr_05,
                              . ~ . -Urb_score:Transect_ID)

# all main effects
u_peds_glm_subtr_09 <- update(u_peds_glm_subtr_08,
                              . ~ . -Urb_score:Year)
...
##### City_dist
```{r}
peds_glm_subtr_list <- list(peds_glm_subtr_01,
                            peds_glm_subtr_02,
                            peds_glm_subtr_03,
                            peds_glm_subtr_04,
                            peds_glm_subtr_05,
...
for (i in 1:length(peds_glm_subtr_list)) {
  formulas_peds[i, 1] <- toString(i)
  formulas_peds[i, 2] <- toString(formula(peds_glm_subtr_list[[i]]))
  # formulas_peds[i, 3] <- r.squaredGLMM(peds_glm_subtr_list[[i]])[1,1] %>%
  #   round(digits = 3)
  # formulas_peds[i, 4] <- r.squaredGLMM(peds_glm_subtr_list[[i]])[1,2] %>%
  #   round(digits = 3)
}

glance_peds <- do.call(rbind,
                       lapply(peds_glm_subtr_list,
                              broom::glance)) %>%
  as.data.frame() %>%
  mutate(BestModels = case_when(
    AIC == min(AIC, na.rm=T) ~ "*",
...
bound_peds <- bind_cols(formulas_peds,
  ...
  view() # Models 3,4,6,7,8,9 have lowest AIC (9 is best)


summary(peds_glm_subtr_09)
peds_glm_subtr_09_anova <- car::Anova(peds_glm_subtr_09) %T>%
  print() # City_dist and transect sig
...
##### Urb_score
```{r}
u_peds_glm_subtr_list <- list(u_peds_glm_subtr_01,
                            u_peds_glm_subtr_02,
                            u_peds_glm_subtr_03,
                            u_peds_glm_subtr_04,
                            u_peds_glm_subtr_05,
...
for (i in 1:length(u_peds_glm_subtr_list)) {
  formulas_peds[i, 1] <- toString(i)
  formulas_peds[i, 2] <- toString(formula(u_peds_glm_subtr_list[[i]]))
  # formulas_peds[i, 3] <- r.squaredGLMM(u_peds_glm_subtr_list[[i]])[1,1] %>%
  #   round(digits = 3)
  # formulas_peds[i, 4] <- r.squaredGLMM(u_peds_glm_subtr_list[[i]])[1,2] %>%
  #   round(digits = 3)
}

glance_peds <- do.call(rbind,
                       lapply(u_peds_glm_subtr_list,
                              broom::glance)) %>%
  as.data.frame() %>%
  mutate(BestModels = case_when(
    AIC == min(AIC, na.rm=T) ~ "*",
...
bound_peds <- bind_cols(formulas_peds,
                        glance_peds %>%
                          dplyr::select(AIC, BestModels)) %T>%
  view() # Models 6,7,8,9 have lowest AIC (9 is best)


summary(u_peds_glm_subtr_09)
u_peds_glm_subtr_09_anova <- car::Anova(u_peds_glm_subtr_09) %T>%
...
##### City_dist: 
```{r}
poll_glm_gradient_01 <- glmmTMB(Peduncles ~ City_dist * Year, 
                data = fertile_pops_all,
                family = "poisson")

plot(simulateResiduals(
poll_glm_gradient_01
)) # also overdispersed... try neg binomial


# plot(simulateResiduals(
# update(poll_glm_gradient_01, .~., family = nbinom1())
# )) # better... try squaring now
# 
# 
# plot(simulateResiduals(
# update(poll_glm_gradient_01, Peduncles^2 ~., family = nbinom1())
# )) # didn't help. going with model above


# Full model
poll_glm_gradient_01 <- update(poll_glm_gradient_01, .~., family = nbinom1())

# Reduced model
poll_glm_gradient_02 <- update(poll_glm_gradient_01, .~ City_dist + Year)
