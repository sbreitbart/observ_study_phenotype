...
lapply(alt_best_mods_Q2.dist_list_lm, tidy_sb) %>%
    lapply(., as.data.frame) %>%
    purrr::reduce(full_join, by = "Predictor") %>%
    arrange(factor(Predictor, levels = c("Distance to City Center",
                                         "Urbanization Score",
  ...
  add_footer_lines("**: p-value <= 0.01") %>%
  add_footer_lines("***: p-value <= 0.001") %>%
  save_as_image(., path = here::here("./Figures_Tables/Q1_Q2_ANOVA/Supplement/Linear_Models/Alternative_Models/Q2_Dist_ANOVA_altmodels_lm.png"))
